...
def reset_password_request():
    ...
    # once the form has been validated (i.e. information has been entered)
    if form.validate_on_submit():
        user = User.query.filter_by(email=form.email.data).first()
        # checks if the user is in the system 
        if user:
            send_password_reset_email(user)
        # isn't part of the if condition because I want this to run regardless
        # this is so the clients cannot use the form to figure out 
        #       if a given user is a member or not
